...
static void eigen_decomposition(double *H, int n, regularize_hessian_mem *mem)
{
    double *V = mem->V;
    double *d = mem->d;
...
static void reconstruct_A(double *H, int n, regularize_hessian_mem *mem)
{
    double *V = mem->V;
    double *d = mem->d;
...
int regularize_hessian_calculate_mem_size(int n) {
    ...
    // d: n
    // e: n
    return (sizeof(regularize_hessian_mem) + 2*n*sizeof(double) + n*n*sizeof(double));
}
